...
| `PATCH`     | `doPost()` *(manual)* | Actualización parcial               | ✅ Sí         |
...
#### ⚠️ **Nota — PATCH y el método `service()`:**

El Servlet API no tiene un `doPatch()` nativo, lo que presenta un dilema:

- La recomendación general es **nunca sobrescribir `service()`**, porque es el dispatcher
  interno que enruta automáticamente hacia `doGet()`, `doPost()`, etc. Si lo sobrescribes
  sin llamar a `super.service()`, rompes ese mecanismo.

- Sin embargo, para soportar `PATCH` de forma nativa en el Servlet API puro, la única
  salida técnica es sobrescribir `service()` con mucho cuidado, detectando el método
  manualmente y delegando el resto a `super.service()`:
...
protected void service(HttpServletRequest request, HttpServletResponse response)
    ...
    if ("PATCH".equalsIgnoreCase(request.getMethod())) {
        doPatch(request, response); // tu propio método custom
    } else {
        super.service(request, response); // ✅ el resto sigue funcionando normal
    }
}
...
> La clave está en **siempre llamar a `super.service()`** para el resto de métodos.
> En la práctica, `PATCH` en Servlet puro es raro. En `JAX-RS` y `Quarkus`, esto
> desaparece por completo: simplemente usas la anotación `@PATCH` y listo. ✅
