# 🏗️ Proyecto de Práctica - Sin CDI
...
public class ShoppingCart {
    ...
    // Establece la cantidad del ítem asociado al producto indicado
...
En cambio, con **Tomcat standalone**, es decir, con Apache Tomcat instalado y ejecutándose de forma independiente (en
nuestra máquina local o en un servidor, y no embebido dentro de una aplicación), el servidor gestiona las conexiones
...
> `DatabaseConnectionFilter`, lo cual es el comportamiento lógico esperado — primero verificar autenticación y luego
> abrir
...
## Desplegando proyecto sin CDI en Tomcat

Llegó el momento de probar nuestra aplicación, así que como estamos trabajando con un proyecto del tipo `war`,
necesitamos desplegar nuestro empaquetado al servidor de tomcat y luego interactuar y ver su funcionamiento.

### Levantando Tomcat

Ejecutamos el siguiente comando para levantar tomcat. Este servidor lo tenemos dentro del disco C, tal como se muestra
a continuación:
...
Using CATALINA_OPTS:   ""
17-May-2026 20:12:37.517 INFO [main] org.apache.catalina.startup.VersionLoggerListener.log Server version name:   Apache Tomcat/11.0.18
17-May-2026 20:12:37.520 INFO [main] org.apache.catalina.startup.VersionLoggerListener.log Server built:          Jan 23 2026 10:22:57 UTC
17-May-2026 20:12:37.520 INFO [main] org.apache.catalina.startup.VersionLoggerListener.log Server version number: 11.0.18.0
17-May-2026 20:12:37.521 INFO [main] org.apache.catalina.startup.VersionLoggerListener.log OS Name:               Windows 11
...
17-May-2026 20:12:40.659 INFO [main] org.apache.coyote.AbstractProtocol.start Starting ProtocolHandler ["http-nio-8080"]
17-May-2026 20:12:40.681 INFO [main] org.apache.catalina.startup.Catalina.start Server startup in [2887] milliseconds
````

### Configurando arranque de la aplicación

La siguiente imagen muestra la configuración de arranque para nuestra aplicación que hacemos en IntelliJ IDEA en
`Edit Configurations...`:

![03.png](assets/02-cdi/01-without-cdi/01.png)

### Limpieza previa antes del despliegue

Para evitar cualquier error en el despliegue vamos a limpiar nuestro proyecto `02-cdi/01-web-app-without-cdi`, así
que nos posicionamos en ese proyecto y ejecutamos el siguiente comando:
...
Una vez que ya tenemos configurado el arranque de nuestra aplicación, ejecutamos el `Run '01-web-app-without-cdi'`
o el `Debug '01-web-app-without-cdi'` desde el `Run / Debug Configurations` de `IntelliJ IDEA`.

Al finalizar el despliegue, veremos en la consola del ide parte del log que ha generado:
...
...
[INFO] 
[INFO] --- war:3.4.0:war (default-war) @ 01-web-app-without-cdi ---
[INFO] Packaging webapp
[INFO] Assembling webapp [01-web-app-without-cdi] in [D:\programming\java\jakarta-ee-essentials\02-cdi\01-web-app-without-cdi\target\01-web-app-without-cdi-1.0-SNAPSHOT]
[INFO] Processing war project
[INFO] Copying webapp resources [D:\programming\java\jakarta-ee-essentials\02-cdi\01-web-app-without-cdi\src\main\webapp]
[INFO] Building war: D:\programming\java\jakarta-ee-essentials\02-cdi\01-web-app-without-cdi\target\01-web-app-without-cdi-1.0-SNAPSHOT.war
[INFO] 
[INFO] <<< tomcat7:2.2:redeploy (default-cli) < package @ 01-web-app-without-cdi <<<
[INFO] 
[INFO] 
[INFO] --- tomcat7:2.2:redeploy (default-cli) @ 01-web-app-without-cdi ---
[INFO] Deploying war to http://localhost:8080/01-web-app-without-cdi  
Uploading: http://localhost:8080/manager/text/deploy?path=%2F01-web-app-without-cdi&update=true
Uploaded: http://localhost:8080/manager/text/deploy?path=%2F01-web-app-without-cdi&update=true (5882 KB at 27104.0 KB/sec)

[INFO] tomcatManager status code:200, ReasonPhrase:
[INFO] OK - Deployed application at context path [/01-web-app-without-cdi]
[INFO] ------------------------------------------------------------------------
[INFO] BUILD SUCCESS
[INFO] ------------------------------------------------------------------------
[INFO] Total time:  5.091 s
[INFO] Finished at: 2026-05-17T20:13:16-05:00
[INFO] ------------------------------------------------------------------------
````

### Accediendo a index.jsp

Si accedemos a la url `http://localhost:8080/01-web-app-without-cdi/` veremos que la vista `index.jsp` se ha desplegado
correctamente como una vista html donde solo vemos el menú de opciones:

![02.png](assets/02-cdi/01-without-cdi/02.png)

En la consola de Tomcat se muestra el siguiente log:

````bash
17-May-2026 20:14:10.243 INFO [http-nio-8080-exec-5] dev.magadiflo.app.db.DataSourceConnectionFactory.getConnection Estableciendo conexi├│n a la base de datos con context.xml
17-May-2026 20:14:10.605 INFO [http-nio-8080-exec-5] dev.magadiflo.app.filter.DatabaseConnectionFilter.doFilter Desactivando autoCommit
17-May-2026 20:14:10.606 INFO [http-nio-8080-exec-5] dev.magadiflo.app.filter.DatabaseConnectionFilter.doFilter Agregando 'connection' al request
17-May-2026 20:14:11.426 INFO [http-nio-8080-exec-5] dev.magadiflo.app.filter.DatabaseConnectionFilter.doFilter Realizando 'commit' en la base de datos 
````

### Listando productos sin estar autenticado

Accedemos a la siguiente url `http://localhost:8080/01-web-app-without-cdi/products`. Vemos que se nos muestran
correctamente los 15 productos que existen en nuestra base de datos.

![03.png](assets/02-cdi/01-without-cdi/03.png)

### Ver carrito sin estar autenticado

Si accedemos al menú ver carrito, cuya url es `http://localhost:8080/01-web-app-without-cdi/carts/view`, veremos que
nos muestra el `mensaje Unauthorized: Debe estar autenticado.`, además se muestra el título
`Estado HTTP 401 – No autorizado`.

![04.png](assets/02-cdi/01-without-cdi/04.png)

### Autenticación de usuario

Nos logueamos con uno de los usuarios que tenemos en la base de datos.

![05.png](assets/02-cdi/01-without-cdi/05.png)

Luego de habernos logueado nos muestra esta vista html correspondiente a la url
`http://localhost:8080/01-web-app-without-cdi/login.html`.

![06.png](assets/02-cdi/01-without-cdi/06.png)

### Ver carrito estando autenticado

Si vamos a la vista de carritos, esta vez vemos que nos pemite entrar y ver que nuestro carrito de compras está vacío
ya que aún no hemos agregado ningún producto.

![07.png](assets/02-cdi/01-without-cdi/07.png)

### Listando productos estando autenticado

Si vamos al listado de productos, estando autenticados, vemos que se nos muestran más opciones, como la columna de
agregar, editar y eliminar, así como el botón de crear.

![08.png](assets/02-cdi/01-without-cdi/08.png)

### Agregando productos al carrito

![09.png](assets/02-cdi/01-without-cdi/09.png)

### Eliminando productos del carrito

![10.png](assets/02-cdi/01-without-cdi/10.png)

![11.png](assets/02-cdi/01-without-cdi/11.png)

### Agregando producto a la lista

![12.png](assets/02-cdi/01-without-cdi/12.png)

### Editando producto de la lista

![13.png](assets/02-cdi/01-without-cdi/13.png)

### Eliminando producto de la lista

![14.png](assets/02-cdi/01-without-cdi/14.png)

### Cerrando sesión

![15.png](assets/02-cdi/01-without-cdi/15.png)
